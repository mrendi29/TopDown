...
//	private boolean outOfBounds=false;
	 ...
	 * @param radius
	 */
	public GameObject(double x, double y, double radius, String imageName) {
		this.x = x;
		this.y = y;
		this.radius = radius;
		
		node = new Circle(x, y, radius);
		playerImg = new Image(imageName);
		imgv = new ImageView(playerImg);
		imgv.setX(x - playerImg.getWidth()/2);
		imgv.setY(y - playerImg.getHeight()/2);
	}
	...
	public void createRandomSpeed() {
		double coef = Math.abs((random.nextDouble()));
